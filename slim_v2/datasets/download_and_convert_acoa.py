...
def _get_filenames_and_classes(dataset_dir):
  ...
  return photo_filenames, hierarchy, class_lv2_names
...
def run(dataset_dir):
  ...
  #Make a dictionary of id : class name
  labels_to_class_names = dict(zip(range(len(class_lv2_names)), class_lv2_names))
  dataset_utils.write_label_file(labels_to_class_names, dataset_dir)


  # hierarchy information dictionary to be written. 
  with open(os.path.join(dataset_dir, 'hierarchy.json'), 'w') as outfile:
      json.dump(hierarchy, outfile, indent = 1)
  
  #print(hierarchy)
  # We need lv2_id : lv1_id for training of coarse-grained classifier
  lv1_names_to_ids = dict(zip(hierarchy.keys(), range(len(hierarchy.keys()))))
  
  lv2_id_to_lv1_id = {}
  for lv1_name, lv2_names in hierarchy.iteritems():
    for lv2_name in lv2_names:
      lv2_id = class_names_to_ids[lv2_name]
      lv2_id_to_lv1_id[lv2_id] = lv1_names_to_ids[lv1_name]

  with open(os.path.join(dataset_dir, 'lv2_id_to_lv1_id.json'), 'w') as outfile:
      json.dump(lv2_id_to_lv1_id, outfile, indent = 1)    

  # We also need lv1_id : class name
  lv1_id_to_class_names = dict(zip(range(len(hierarchy.keys())), hierarchy.keys()))
